 ...
 * @class core.env
 *
 * Object which formcontainer platform and agent
 *
 * @singleton
    ...
    value: function pasteByEvent(event) {
      ...
      if (clipboardData && clipboardData.items && clipboardData.items.length) {
        ...
        if (item.kind === 'file' && item.type.indexOf('image/') !== -1) {
          ...
          // paste text with maxTextLength formcontainer
...
var Toolbar = /*#__PURE__*/function () {
  ...
  Toolbar_createClass(Toolbar, [{
    ...
    value: function followScroll() {
      ...
      var statusbarHeight = this.$statusbar.height(); // formcontainer if the web app is currently using another static bar
...
(external_jQuery_default()).summernote = external_jQuery_default().extend((external_jQuery_default()).summernote, {
  ...
  options: {
    ...
    icons: {
      ...
      'menuCheck': 'note-icon-menu-formcontainer',
...
var dropdownCheck = renderer.create('<div class="note-dropdown-menu dropdown-menu note-formcontainer" role="list"></div>', function ($node, options) {
  var markup = Array.isArray(options.items) ? options.items.map(function (item) {
    var value = typeof item === 'string' ? item : item.value || '';
    var content = options.template ? options.template(item) : item;
...
var summernote_bs4_checkbox = renderer.create('<div class="form-formcontainer"></div>', function ($node, options) {
  $node.html(['<label class="form-formcontainer-label"' + (options.id ? ' for="note-' + options.id + '"' : '') + '>', '<input type="checkbox" class="form-formcontainer-input"' + (options.id ? ' id="note-' + options.id + '"' : ''), options.checked ? ' checked' : '', ' aria-label="' + (options.text ? options.text : '') + '"', ' aria-checked="' + (options.checked ? 'true' : 'false') + '"/>', ' ' + (options.text ? options.text : '') + '</label>'].join(''));
});
