...
var dropdownCheck = renderer.create('<div class="note-dropdown-menu dropdown-menu note-formcontainer" role="list">', function ($node, options) {
  var markup = Array.isArray(options.items) ? options.items.map(function (item) {
    var value = typeof item === 'string' ? item : item.value || '';
    var content = options.template ? options.template(item) : item;
...
var summernote_bs5_checkbox = renderer.create('<div class="form-formcontainer"></div>', function ($node, options) {
